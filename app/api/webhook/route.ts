...
export async function POST(req: Request) {
  ...
  // Post-auth handling touches storage (Redis) — wrap it so a failure logs
  // context and returns 500 (GitHub retries the delivery) instead of a silent
  // 200. Behavior is otherwise identical.
  try {
    ...
    console.error({ evt: event, kind: payload.action ?? 'none', err: message })
